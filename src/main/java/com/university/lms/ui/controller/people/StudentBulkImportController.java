...
import com.university.lms.util.ExcelImportUtil;

/** Bulk student registration from an Excel (.xlsx) file, with a per-row rejected-rows report. */
public final class StudentBulkImportController implements Initializable {
    ...
    public StudentBulkImportController(AppContext appContext) {
    ...
    private void onChooseFile() {
        ...
        chooser.setTitle("Select Student Import Excel File");
        ...
        appContext.getAsyncExecutor().run(
                () -> {
                    try (InputStream inputStream = new FileInputStream(selectedFile)) {
                        List<StudentImportRowDTO> rows = ExcelImportUtil.parseStudentRows(inputStream);
                        return appContext.getStudentService().bulkImport(rows);
                    } catch (IOException e) {
    ...
    private void onImportComplete(ImportResultDTO result) {
        importButton.setDisable(false);
        summaryLabel.setText(result.successCount() + " student(s) imported, "
